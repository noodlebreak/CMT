...
class CMT(object):
    ...
    def initialise(self, im_gray0, tl, br):
        ...
        for k1, i1 in zip(selected_keypoints, range(num_selected_keypoints)):
            for k2, i2 in zip(
                    selected_keypoints, range(num_selected_keypoints)):
    ...
    def estimate(self, keypoints):
        ...
        if keypoints.size > 1:
            ...
            if not all(duplicate_classes):
                ...
                # as it can happen that there is more than one keypoint at a
                # single location
                ...
                angle_diffs[long_way_angles] = angle_diffs[
                    long_way_angles] - np.sign(
    ...
    def process_frame(self, im_gray):
        ...
        tracked_keypoints, _ = util.track(
            self.im_prev, im_gray, self.active_keypoints)
        (center,
         scale_estimate,
         rotation_estimate,
        ...
        matches_all = self.matcher.knnMatch(
            features,
        ...
        if not any(isnan(center)):
            selected_matches_all = self.matcher.knnMatch(
                features, self.selected_features, len(
                    self.selected_features))

        # For each keypoint and its descriptor
        if len(keypoints_cv) > 0:
            transformed_springs = scale_estimate * \
                util.rotate(self.springs, -rotation_estimate)
            for i in range(len(keypoints_cv)):
                ...
                if not any(isnan(center)):
                    ...
                    # Compute the keypoint location relative to the object
                    # center
                    relative_location = location - center

                    # Compute the distances to all springs
                    displacements = util.L2norm(
                        transformed_springs -
                        relative_location)

                    # For each spring, calculate weight
                    # Could be smooth function
        ...
        if not any(isnan(self.center)) and self.active_keypoints.shape[
                0] > self.num_initial_keypoints / 10:
